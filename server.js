...
server.post('/signup', (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    res.status(403).json({
      error:
        'Password should be 3 to 20 characters long with a numeric, 1 lowercase and 1 uppercase letter.',
    });
  }
    ...
    .then((user) => {
      ...
      bcrypt.compare(password, user.personal_info.password, (err, result) => {
        if (err) {
          return res
            .status(403)
            .json({ error: 'Error occurred while login please' });
        }
        if (!result) {
      ...
      // });
      // console.log(user);
      // return res.json({ status: 'Got user documents' });
    })
    .catch((err) => {
